...
description: PyTorch 深度学习模式和最佳实践，用于构建稳健、高效且可复现的训练流程、模型架构和数据加载。
origin: ECC
---

# PyTorch 开发模式

地道的 PyTorch 模式和最佳实践，用于构建稳健、高效且可复现的深度学习应用。
...
始终编写可在 CPU 和 GPU 上运行的代码，不硬编码设备。
...
| `torch.amp.autocast` | 混合精度，速度提升 2 倍 |
...
__记住__：PyTorch 代码应做到设备无关、可复现且关注内存使用。如有疑问，使用 `torch.profiler` 进行分析，并用 `torch.cuda.memory_summary()` 检查 GPU 内存。
