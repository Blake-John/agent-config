...
description: 基于 C++ Core Guidelines (isocpp.github.io) 的 C++ 编码标准。在编写、审查或重构 C++ 代码时使用，以强制执行现代、安全和符合惯用法的实践。
origin: ECC
---

# C++ 编码标准（C++ Core Guidelines）

面向现代 C++（C++17/20/23）的全面编码标准，源自 [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines)。强制执行类型安全、资源安全、不变性和清晰性。
...
3. **类型安全**（P.4, I.4, ES.46-49, Enum.3）：使用类型系统在编译时预防错误
4. **表达意图**（P.3, F.1, NL.1-2, T.10）：名称、类型和概念应传达目的
5. **最小化复杂度**（F.2-3, ES.5, Per.4-5）：简单的代码是正确的代码
6. **值语义优先于指针语义**（C.10, R.3-5, F.20, CP.31）：优先按值返回和作用域对象
...
| **F.8** | 优先使用纯函数 |
| **F.16** | 对于"输入"参数，廉价复制类型按值传递，其他类型使用 `const&` |
| **F.20** | 对于"输出"值，优先使用返回值而非输出参数 |
...
| **C.9** | 最小化成员暴露 |
| **C.20** | 如果能够避免定义默认操作，就不要定义（零规则） |
| **C.21** | 如果定义了或 `=delete` 了任何拷贝/移动/析构函数，请处理它们全部（五规则） |
| **C.35** | 基类析构函数：应为 public virtual 或 protected non-virtual |
| **C.41** | 构造函数应创建完全初始化的对象 |
| **C.46** | 声明单参数构造函数为 `explicit` |
| **C.67** | 多态类应抑制 public 拷贝/移动 |
| **C.128** | 虚函数：精确指定 `virtual`、`override` 或 `final` 其中之一 |
...
| **R.21** | 除非需要共享所有权，否则优先使用 `unique_ptr` 而非 `shared_ptr` |
...
| **ES.20** | 始终初始化对象 |
| **ES.23** | 优先使用 `{}` 初始化语法 |
| **ES.25** | 除非需要修改，否则将对象声明为 `const` 或 `constexpr` |
...
| **Con.4** | 对于构造后不会变化的值使用 `const` |
| **Con.5** | 对于可在编译时计算的值使用 `constexpr` |
...
- [ ] 模板使用 concepts 约束（T.10）
- [ ] 头文件全局作用域中没有 `using namespace`（SF.7）
- [ ] 头文件有包含守卫并且是自包含的（SF.8, SF.11）
