...
构建安全、高性能、可维护应用的惯用 Rust 模式和最佳实践。
...
本 skill 在六个关键领域强制执行惯用 Rust 约定：所有权和借用（ownership and borrowing）在编译时防止数据竞争，使用 `thiserror`（库）和 `anyhow`（应用）进行 `Result`/`?` 错误传播，使用枚举（enums）和穷尽模式匹配（exhaustive pattern matching）使非法状态不可表示，使用 trait 和泛型（generics）实现零成本抽象，通过 `Arc<Mutex<T>>`、通道（channels）和 async/await 实现安全并发，以及按领域组织的最小化 `pub` 表面。

## 核心原则

### 1. 所有权和借用（Ownership and Borrowing）
...
### 使用 `Result` 和 `?` — 生产环境中永远不要使用 `unwrap()`
...
### 穷尽匹配（Exhaustive Matching）— 业务逻辑中不使用通配符
...
### 使用 Trait 对象实现动态分发（Dynamic Dispatch）
...
## 结构体和数据建模（Structs and Data Modeling）
...
| 惯用法 | 描述 |
|-------|------|
| 借用而非克隆 | 除非需要所有权，否则传 `&T` 而非克隆 |
| 使非法状态不可表示 | 使用枚举仅对合法状态建模 |
| `?` 优先于 `unwrap()` | 传播错误，绝不在库/生产代码中 panic |
| 解析而非验证 | 在边界处将非结构化数据转换为类型化结构体 |
| 用 Newtype 保障类型安全 | 将原生类型包装在 newtype 中防止参数互换 |
| 优先使用迭代器而非循环 | 声明式链更清晰且通常更快 |
| 在 Result 上使用 `#[must_use]` | 确保调用者处理返回值 |
| 使用 `Cow` 实现灵活的所有权 | 在借用足够时避免分配 |
| 穷尽匹配 | 业务关键型枚举不使用通配符 `_` |
| 最小化 `pub` 表面 | 内部 API 使用 `pub(crate)` |
...
**记住**：如果编译通过，它很可能是正确的——但前提是你避免使用 `unwrap()`，最小化 `unsafe`，并让类型系统为你工作。
