...
使用 `web_search` 工具进行广度优先搜索：

1. **基础认知**: `"[领域] 入门指南 基础概念 工作原理"` - 获取通俗解释
2. **历史脉络**: `"[领域] 发展历史 里程碑 时间线"` - 梳理演进阶段
3. **核心术语**: `"[领域] 核心概念 关键术语 定义"` - 提取关键词
4. **当前动态**: `"[领域] 最新进展 2024 2025 趋势"` - 捕捉前沿
5. **权威来源**: 识别该领域的顶级会议、期刊、权威机构、活跃研究者
...
基于 Phase 2 提取的关键词，使用 `paper_search_skill` 进行系统性检索：
...
**输出**: 生成 `phase2_papers.json`，包含文献元数据（标题、作者、年份、摘要、PDF链接、重要性评级、所属流派）
...
1. **批量下载**: 使用工具下载 P0/P1 文献 PDF 到 `./research_papers/` 目录
2. **结构化精读**: 对每篇核心文献，创建笔记 `phase3_notes/author_year_title.md`：
...
5. **记忆更新**: 使用 `memory_space_edits` 记录用户学习进度与兴趣点

---

## 质量控制标准

- **引用密度**: 每个核心论点有文献支撑，报告至少20处引用
...
- `web_search`: 获取领域概览、最新动态、非学术资源
- `paper_search_skill`: 检索学术数据库（arXiv、Google Scholar、IEEE、ACM）
- `file_operations`: 创建目录结构、管理文献与笔记
- `pdf_reader`: 解析下载的学术文献
- `ipython`: 生成知识图谱可视化、数据分析图表
- `memory_space_edits`: 记录用户偏好与学习历史
